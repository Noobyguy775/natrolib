...
nm_status(status)
{
	...
	if (discordCheck = 1)
	{
		...
		content := ((criticalCheck = 1) && discordUID 
			...
			|| ((state = "Reporting") && InStr(stateString, "Beequip")) || (state = "Failed") && InStr(stateString, "Beequip") || (state = "Opened"))
			? ("<@" discordUID ">") : ""
...
nm_command(command)
{
	...
	switch % (name := params[1])
	{
		...
		case "beequip","beequips":
			if (params[3] => 6 || params[3] =< 0) {
				discord.SendEmbed("Incorrect slot. Slots 1-6 are valid.", 16711731, , , , id)
				return
				}
			DetectHiddenWindows, On
			SetTitleMatchMode, 2
			switch % params[2] {
				case "buy", "purchase":
					...
					SetTitleMatchMode, 2
					if WinExist("natro_macro.ahk ahk_class AutoHotkey")
						PostMessage, 0x5559, params[3], 1
					else {
						discord.SendEmbed("Macro window not found")
					}
				case "view", "see":
					if (params[3] => 6 || params[3] =< 0) {
						discord.SendEmbed("Incorrect slot. Slots 1-6 are valid.", 16711731, , , , id)
					...
					SetTitleMatchMode, 2
					if WinExist("natro_macro.ahk ahk_class AutoHotkey")
						PostMessage, 0x5559, params[3], 2
					else {
						discord.SendEmbed("Macro window not found")
					}
				case "storage":
					if WinExist("natro_macro.ahk ahk_class AutoHotkey")
						PostMessage, 0x5559, params[3], 3
					else {
						discord.SendEmbed("Macro window not found")
					}
			}
